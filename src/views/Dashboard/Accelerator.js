...
function Accelerator({ pedal, setPedal }) {
  const [openThrottlePercent, setOpenThrottlePercent] = useState(0);
  const keyFunction = useCallback(
    (event) => {
      if (event.keyCode === 68) {
        if (openThrottlePercent >= 4) {
          setOpenThrottlePercent(openThrottlePercent - 4);
        }
      }
      if (event.keyCode === 67) {
        if (openThrottlePercent <= 96) {
          setOpenThrottlePercent(openThrottlePercent + 4);
        }
      }
    },
